...
- **workflow-guard** ([workflow-guard.md](../workflow-guard/agents/workflow-guard.md)): Hook management plugin with a `templates/` directory for shipping pre-built guard scripts. Good example of a plugin that uses the templates pattern, safe settings.json merging, and the `/guard` skill for interactive guard lifecycle management.
